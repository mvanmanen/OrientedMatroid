...
We have that $P_1,\cdots , P_d$ are $d$ points in $\RR^n$, with spheres with radius $r_i$, $i=1,\cdots , d$  around them
We want to compute the center of the spheres that touches these spheres, and that lies in the affine plane of dimension $d-1$ through these points.
We need $d\leq n+1$.
...
For the point $Q$ to be the center of the sphere the functions $f_i(Q)$ all need to be equal and the function 
$f(x)=\max_if_i(x)$ needs to be at a maximum point at $x=q$.
That means that $g(x)=\frac12\lVert x\rVert^2-f(x)$ needs to be a critical point $x=q$.
